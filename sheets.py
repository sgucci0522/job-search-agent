...
# 結果シートの列定義
RESULT_COLUMNS = ['求人サイト', 'タイトル', '会社名', '勤務地', '雇用形態', '給与', '応募期限', 'URL', '取得日時', 'リモート判定', '応募状況']
COL_STATUS = 10  # K列（0始まり）= 応募状況
COL_URL = 7      # H列（0始まり）= URL
...
def write_results(jobs: list[dict]):
    ...
    kept_urls = {row[COL_URL] for row in kept_rows if len(row) > COL_URL and row[COL_URL]}
    ...
    new_rows = [
        ...
        for job in jobs
        if job.get('url') not in kept_urls
    ]
